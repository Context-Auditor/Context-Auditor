...
class HTMLParser:
    ...
    def findXSSAttackOneReqTamper(self, Mal, MalQueryParams):
        # return 404
        st = ''
        for key in MalQueryParams.keys():
            # ************** INJECTIONS INSIDE JAVASCRIPT************************
            malString = unquote_plus(unquote_plus(MalQueryParams[key]))
            MalScriptStartIndex = Mal.lower().find("<script")
            MalScriptEndIndex = Mal.lower().find("</script>")
            MalStyleStartIndex = Mal.lower().find("<style")
            MalStyleEndIndex = Mal.lower().find("</style>")
            MalParameterIndex = Mal.find(malString)
            if (MalScriptStartIndex < MalParameterIndex < MalScriptEndIndex):

                MalScriptTemp = Mal[MalScriptStartIndex + 1:MalScriptEndIndex]
                #MalScriptTemp=
                MalScript = MalScriptTemp[MalScriptTemp.find('>') + 1:]
                #
                #dummyStart=MalScript.find("<!--")
                #dummyEnd=MalScript.find('-->')
                #while(dummyStart>0):
                 #   if(dummyEnd>0):
                #
                  #      MalScript.replace(MalScript[dummyStart:dummyEnd+2],'')
                   # else:
                    #     MalScript=''
                     #    dummyStart = MalScript.find("<!--")
                      #   dummyEnd = MalScript.find('-->')
                ...
                if (expectedState != -1 and len(expectedState)>2):
                    # return 404
                    # return 300
                    ...
                    preState = expectedState[1]
                    for state in expectedState[2:]:
                        # print state
                        if (state != preState):
                            st+="An injection inside JavaScript found for param : "+ key + "and value: " + malString + '\n'
                            st+= "***************** State Transitions from: " + preState + " To " + state + " \n***********"
                            return st
                            break
                        preState = state

            elif (MalStyleStartIndex < MalParameterIndex < MalStyleEndIndex):

                MalScriptTemp = Mal[MalStyleStartIndex:MalStyleEndIndex]
                MalScript = MalScriptTemp[MalScriptTemp.find('>') + 1:]
                dummyStart=MalScriptTemp.find("<!--")
                dummyEnd=MalScriptTemp.find('-->')
                if(dummyStart>0):
                    if(dummyEnd>0):
                        MalScript=MalScriptTemp[dummyEnd+3:]
                    else:
                        MalScript=''
                self.CSSparser.restart(MalScript)
                expectedState = self.CSSparser.states_at_Malicious_substring(malString)
                if (expectedState != -1 and len(expectedState)>2):
                    # return 404
                    # return 300
                    # print "We Found parsing states for desired input as: " + MalQueryParams[key] + " is: " + expectedState
                    ...
                    for state in expectedState[2:]:
                        ...
                        if (state != preState):
                            #st+= "***************** State Transitions from: " + preState + " To " + state + " \n***********"
                            st += "An injection inside CSS found for param : " + key + " and value: " + malString + "  \n"
                            return st
                            break
                        preState = state
                    # return 200
            # return (200 , st)
                ...
                if (expectedState != -1):
                    ...
                    for state in expectedState:
                        ...
                        if (state != preState):
                            #st+= "***************** State Transitions from: " + preState + " To " + state + " \n***********"
                            st += "An injection inside HTML found for param : " + key + " and value: " + malString + '\n'
                            return st
    ...
    while 1:
        ...
        if message == None:
            ...
            if (message['method'] == 'hello'):
                ...
                send_message(json.dumps({'method': 'log', 'message': 'Starting proxy YESSSS... (' + str(message['port']) + ')'}))
...
class InjectingMaster(flow.FlowMaster):
    ...
    def handle_request(self, msg):
        ...
        state['req']=fullURL
        send_message(json.dumps({'method': 'log', 'message': fullURL + "HEREE"}))

        if (f.request.host == 'mitm.it'):
	    send_message(json.dumps({'method': 'log', 'message': "2"}))
            path = f.request.path
            ...
            try:
                ...
                responseHeaders['CA'] = 'ANALYZED'
                send_message(json.dumps({'method': 'log', 'message': "HERRREEE" }))
                resp = HTTPResponse([1,1], 200, 'OK', responseHeaders, content)
                msg.reply(resp)
            except:
                resp = HTTPResponse([1,1], 404, 'Not Found', ODictCaseless([]), '')
                msg.reply(resp)
	send_message(json.dumps({'method': 'log', 'message': "HEREE3"}))
	#send_message(json.dumps({'method': 'log', 'message': resp }))

        for url in urlsToProxy:
	    send_message(json.dumps({'method': 'log', 'message': "HERRREEE Cached" }))
		...
		responseHeaders.append(['CAAAA', 'tamper'])
		send_message(json.dumps({'method': 'log', 'message': type(content) + "HEREE2222"}))
		send_message(json.dumps({'method': 'log', 'message': "HEREE6"}))
                resp = HTTPResponse([1,1], 200, 'OK', ODictCaseless(responseHeaders), content)
                msg.reply(resp)
                break

        ##send_message(json.dumps({'method': 'log', 'message': content + "HEREE2222"}))
	...
	send_message(json.dumps({'method': 'log', 'message': state['req'] + "HEREE4" }))
        f.response.stream = True
	#f.response.headers['Proxy-Connection'] = ['keep-alive']
        f.reply()
	return f
    ...
    def handle_response(self, msg):
	    ...
	    with flow.response:
            #if f.response.body:
            	URL=state['req']
	    	state['resp']=f.response.content
	    	send_message(json.dumps({'method': 'log', 'message': state['resp'] + "yesss"}))
            	#msg.reply()
	    	parser = HTMLParser.HTMLParser()
            	if ("?" in URL):

        		html = state['res'].decode("utf-8")
        		...
        		if (len(queries) > 1):
            			...
            			result = parser.findXSSAttackOneReqTamper(html, params)

            			r=''
            			if (result != ''):
            		    		r += (result + '\n')
            	   			r += ("Found Context Injection URL : " + URL + '\n')
					send_message(json.dumps({'method': 'log', 'message': r +"RRRRRR" }))
